...
export async function PUT(
  ...
  const mapped: any = {
    ...
    bhk: data.bhk
      ? data.bhk === "Studio"
        ? "Studio"
        : ({ "1": "One", "2": "Two", "3": "Three", "4": "Four" } as const)[data.bhk]
      : null,
  ...
  const updated = await prisma.$transaction(async (tx) => {
    const buyer = await tx.buyer.update({ where: { id }, data: mapped });
    const diff: Record<string, any> = {};
    for (const k of Object.keys(mapped)) {
      if ((existing as any)[k] !== (buyer as any)[k])
        diff[k] = { from: (existing as any)[k], to: (buyer as any)[k] };
    }
    await tx.buyerHistory.create({
      data: { buyerId: buyer.id, changedBy: session.user!.id as string, diff },
    });
    return buyer;
  });
